...
# 查看缓存统计
curl -X POST http://localhost:8000/mcp \
  -d '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"get_server_metrics","arguments":{}}}'

# 清空缓存
curl -X POST http://localhost:8000/mcp \
  -d '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"clear_cache","arguments":{}}}'
```
...
for endpoint, ep_stats in stats['endpoint_stats'].items():
    print(f"{endpoint}: P95={ep_stats['p95']:.3f}s")
```
