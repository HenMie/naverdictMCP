...
async def get_server_metrics() -> str:
    ...
    Returns:
        JSON formatted metrics including:
        - Request statistics (total, success rate, error counts)
...
async def clear_cache() -> str:
    ...
    Returns:
        JSON formatted confirmation message
    """
    logger.info("清空缓存")
    old_size = cache.size()
    cache.clear()
...
async def health_check() -> str:
    ...
    Returns:
        JSON formatted health status including basic metrics
    """
    stats = metrics.get_stats()
    ...
    return json.dumps({
        ...
        "version": "0.1.0",
        "uptime_requests": stats["total_requests"],
